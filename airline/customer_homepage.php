...
<body>
    ...
    <?php
    // Display welcome message and frequent flyer details
    echo "<h2>Welcome " . $_SESSION['login_user'] . "</h2>";

    // Database connection and query
    require_once('Database Connection file/mysqli_connect.php');

    $query = "SELECT frequent_flier_no, mileage FROM Frequent_Flier_Details WHERE customer_id = ?";
    $stmt = mysqli_prepare($dbc, $query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "s", $_SESSION['login_user']);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $frequent_flier_no, $mileage);

        // Fetch the first row (assuming only one row expected)
        mysqli_stmt_fetch($stmt);

        // Display frequent flyer details
        if ($frequent_flier_no) {
    ...
    <table cellpadding="5">
        ...
        <tr>
            <td class="admin_func"><a href="view_booked_tickets.php"><i class="fa fa-plane" aria-hidden="true"></i> View Booked Flight Tickets</a></td>
        </tr>
        <tr>
            <td class="admin_func"><a href="pnr.php"><i class="fa fa-plane" aria-hidden="true"></i> Print Booked Ticket</a></td>
        </tr>
        <tr>
            <td class="admin_func"><a href="cancel_booked_tickets.php"><i class="fa fa-plane" aria-hidden="true"></i> Cancel Booked Flight Tickets</a></td>
        </tr>
    </table>
    <footer>
